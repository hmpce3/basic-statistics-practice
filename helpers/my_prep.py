...
def log_transfrom(df, log_columns=None, reflect_columns=None, verbose=True):
    ...
    # 값의 대소 관계가 뒤집히므로 회귀계수의 부호도 반대로 해석해야한다
    if reflect_columns:
        for c in reflect_columns:
...
def inverse_log_transform(df, log_columns=None, reflect_columns=None, verbose=True):
    ...
    Args:
        ...
        log_columns (list, optional) : 로그 변환했던 (우측꼬리) 컬럼명 리스트(기본값 : None)
        reflect_columns (dict, optional): 반사 변환했던(좌측꼬리 컬럼의 {컬럼명: 변환 당시의 최댓값}) (기본값 : None)
        verbose (bool) : 컬럼명 역변환식과 값의 범위 변화를 출력할지 여부 (기본값: True)
    ...
    if reflect_columns:
        for c, max_value in reflect_columns.items():
            ...
            report.append([c, '좌측꼬리', f'{max_value:g}-(exp(y)-1)'])
